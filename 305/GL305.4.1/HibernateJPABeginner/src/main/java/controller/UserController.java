...
//        namedQueryExample(session);
    ...
    public static void addUser(SessionFactory factory, Session session) {
        ...
        User uFour = new User("Christ", "christ@gmail.com",
                "147852", 35, 35000.3, "NJ");
        User uFive = new User("Sid", "Sid",
                "s258", 29, 4000.36, "LA");
        //Integer  userid = null;
...
//        String hqlFrom = "FROM User"; // Example of HQL to get all records of user class
        ...
        for (User u:results) {
            System.out.printf(" %-10d %-20s %-30s %s %n",
    ...
    public static void getMaxSalaryGroupBy(Session session) {
        ...
        TypedQuery<Object[]> query = session.createQuery(hql, Object[].class);
        List<Object[]> results = query.getResultList();
        System.out.printf("%s%13s%n","Total Salary","City");
        for (Object[] a : results) {
            System.out.printf("%-20s%-13s%n",a[0],a[1]);
        }
    }
    ...
    public static void namedQueryExample(Session session) {
        ...
        System.out.printf("%s%13s%17s%34s%21s%n", "|User Id", "|Full name",
